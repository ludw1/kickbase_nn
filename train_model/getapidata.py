...
for num,id in enumerate(knownIds):
    ...
    if response.status_code == 200 and response2.status_code == 200 and response3.status_code == 200:
        ...
        last_20_values = [] # list to store the last 20 market values
        stagant = False # flag to stop the loop if the market values are stagnant
        ...
        for dict_1 in value_data['marketValues']:
            ...
            price = dict_1['m']
            last_20_values.append(price)
            if len(last_20_values) > 20:
                last_20_values.pop(0)
                if np.std(last_20_values) < 1e-06:
